...
class UserProfile(models.Model):
	...
	minerid =  models.IntegerField("矿工号",default=10000,validators=[MaxValueValidator(339),MinValueValidator(1)],unique=True)
...
#	lastname = models.CharField(_('lastname'),max_length=16)
	...
	phone = models.CharField("电话",max_length=24,default='not specified')
	...
	astockfund = models.IntegerField('现持有A股基金(份)',default=0,validators=[MaxValueValidator(20000000),MinValueValidator(100000)])
	...
	def user_in_astockfund_group(self):
		...
		return self.user.username == '9818674' or gasharefund in self.user.groups.all()
...
#		return self.sha256_first + self.sha256_second > Decimal('0.0')
	...
	def get_sum_queryset_astockfund(self):
		return UserProfile.objects.filter(astockfund__gt=2000).aggregate(Sum('astockfund'),Sum('hitechfund'),Sum('coinfund'),Sum('wellcoin'))
	...
	def sum_sha256_first(self):
		gminer = Group.objects.get(id=3)
		return UserProfile.objects.filter(user__groups=gminer).aggregate(Sum('sha256_first'))['sha256_first__sum']
	...
	def sum_sha256_second(self):
		gminer = Group.objects.get(id=3)
		return UserProfile.objects.filter(user__groups=gminer).aggregate(Sum('sha256_second'))['sha256_second__sum']
...
#		labels = [ 'bid', 'ask','high','low' ]
		for rp in ReportBitStamp.objects.filter(reporttime__gt=datetime.datetime(2014,6,15,12)):
...
#		jqplotdata = [jqplotdata2,jqplotdata3,jqplotdata4,jqplotdata5]
		title = 'bitstamp graphing'
		plotid = re.sub(r' ','-',title)
		jqplots += [ [plotid, gen_dateline_js(plotid=plotid,jqplotdata=jqplotdata,title=(title).upper(),labels=labels,forceTickAt0=False,forceTickAt100=False, width='1400px', fill=[0,1])] ]
		return jqplots
